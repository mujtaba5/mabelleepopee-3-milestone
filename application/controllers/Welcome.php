...
class Welcome extends CI_Controller {
	...
	public function mapGenerator(){
		$data['gpxTrackList'] = $this->model_gpx_track->getGpxTrackList();
		$gpxTrackList=$data['gpxTrackList'];
		if($gpxTrackList){
			foreach($gpxTrackList as $list){
				// echo base_url().GPX_FILE_PATH.'/'.$list->gpx_file_path;
			}
		}
	...
	public function convertGpxFileToGeoJson(){
		...
		if(isset($post['gpx_track']) && !empty($post['gpx_track'])){
			$inputFileData  = $post['gpx_track'];
			$bool 		    = true;
		}
